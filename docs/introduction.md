...
:::note
Uptick ID project is forked from PrivadoID project under open source license. You can find more information about the PrivadoID project code and document at https://github.com/0xPolygonID/docs and https://docs.privado.id/
:::


Uptick ID’s identity infrastructure is forked from PrivadoID project under open source license, facilitating trusted and secure relationships between apps and users, and following the principles of self-sovereign identity and privacy by default. Uptick ID enables organizations on one side to issue verifiable credentials on the Uptick chain, about users, and organizations, on the other side, to verify those claims via a suite of tools created for each member of the SSI ecosystem.
## Why choose PrivadoID project?

PrivadoID, with the help of zero-knowledge proofs, lets users prove their identity without the need to expose their private information. This ensures both the Freedom of Expression and Privacy by Default (user's identities are secured by zero-knowledge cryptography).
## Core Concepts Verifiable Credentials, Identity Holder, Issuer and Verifier
...
The toolset made available by 0xPrivadoID is fully compliant with the W3C standards. [<ins>You can check the detailed docs here.</ins>](https://github.com/0xPolygonID/did-polygonid).
...
## What Can you Achieve Using Uptick ID on Uptick chain?
